...
/**
 * La classe Maze permet de créer la carte qui sera utilisée aussi bien pour la
 * vue des utilisateurs que pour le contrôle du jeu Sur la carte, on définit les
 * murs, le placement des capsules et gommes, ainsi que les positions des
 * différents agents du jeu au début
 */
public class Maze implements Serializable {
            ...
            while ( ( ligne = br.readLine() ) != null ) {
                ...
                for ( int x = 0; x < ligne.length(); x++ ) {
                    if ( ligne.charAt( x ) == '%' )
                        ...
                        walls[x][y] = false;
                    if ( ligne.charAt( x ) == '.' )
                        foods_start[x][y] = true;
                    else
                        foods_start[x][y] = false;
                    if ( ligne.charAt( x ) == 'o' )
                        capsules_start[x][y] = true;
                    else
                        capsules_start[x][y] = false;
                    if ( ligne.charAt( x ) == 'P' ) {
                        pacman_start.add( new PositionAgent( x, y, PositionAgent.Dir.NORTH ) );
                    }
                    if ( ligne.charAt( x ) == 'G' ) {
                        ghosts_start.add( new PositionAgent( x, y, PositionAgent.Dir.NORTH ) );
                    }
                }
                y++;
            }
    ...
    public void setFoods( boolean[][] foodsNew ) {
        foods = foodsNew;
    }
